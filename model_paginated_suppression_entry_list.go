/*
Pidgin Host API

The Pidgin Host API allows you to manage Services and resources within the Pidgin Host cloud in a simple, programmatic way using conventional HTTP requests. All of the functionality that you are familiar with in the Pidgin Host control panel is also available through the API, allowing you to script the complex actions that your situation requires.

API version: 1.0.0 (v1)
Contact: support@pidginhost.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package pidginhostsdk

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the PaginatedSuppressionEntryList type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PaginatedSuppressionEntryList{}

// PaginatedSuppressionEntryList struct for PaginatedSuppressionEntryList
type PaginatedSuppressionEntryList struct {
	Count int32 `json:"count"`
	Next NullableString `json:"next,omitempty"`
	Previous NullableString `json:"previous,omitempty"`
	Results []SuppressionEntry `json:"results"`
}

type _PaginatedSuppressionEntryList PaginatedSuppressionEntryList

// NewPaginatedSuppressionEntryList instantiates a new PaginatedSuppressionEntryList object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPaginatedSuppressionEntryList(count int32, results []SuppressionEntry) *PaginatedSuppressionEntryList {
	this := PaginatedSuppressionEntryList{}
	this.Count = count
	this.Results = results
	return &this
}

// NewPaginatedSuppressionEntryListWithDefaults instantiates a new PaginatedSuppressionEntryList object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPaginatedSuppressionEntryListWithDefaults() *PaginatedSuppressionEntryList {
	this := PaginatedSuppressionEntryList{}
	return &this
}

// GetCount returns the Count field value
func (o *PaginatedSuppressionEntryList) GetCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Count
}

// GetCountOk returns a tuple with the Count field value
// and a boolean to check if the value has been set.
func (o *PaginatedSuppressionEntryList) GetCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Count, true
}

// SetCount sets field value
func (o *PaginatedSuppressionEntryList) SetCount(v int32) {
	o.Count = v
}

// GetNext returns the Next field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PaginatedSuppressionEntryList) GetNext() string {
	if o == nil || IsNil(o.Next.Get()) {
		var ret string
		return ret
	}
	return *o.Next.Get()
}

// GetNextOk returns a tuple with the Next field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PaginatedSuppressionEntryList) GetNextOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Next.Get(), o.Next.IsSet()
}

// HasNext returns a boolean if a field has been set.
func (o *PaginatedSuppressionEntryList) HasNext() bool {
	if o != nil && o.Next.IsSet() {
		return true
	}

	return false
}

// SetNext gets a reference to the given NullableString and assigns it to the Next field.
func (o *PaginatedSuppressionEntryList) SetNext(v string) {
	o.Next.Set(&v)
}
// SetNextNil sets the value for Next to be an explicit nil
func (o *PaginatedSuppressionEntryList) SetNextNil() {
	o.Next.Set(nil)
}

// UnsetNext ensures that no value is present for Next, not even an explicit nil
func (o *PaginatedSuppressionEntryList) UnsetNext() {
	o.Next.Unset()
}

// GetPrevious returns the Previous field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PaginatedSuppressionEntryList) GetPrevious() string {
	if o == nil || IsNil(o.Previous.Get()) {
		var ret string
		return ret
	}
	return *o.Previous.Get()
}

// GetPreviousOk returns a tuple with the Previous field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PaginatedSuppressionEntryList) GetPreviousOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Previous.Get(), o.Previous.IsSet()
}

// HasPrevious returns a boolean if a field has been set.
func (o *PaginatedSuppressionEntryList) HasPrevious() bool {
	if o != nil && o.Previous.IsSet() {
		return true
	}

	return false
}

// SetPrevious gets a reference to the given NullableString and assigns it to the Previous field.
func (o *PaginatedSuppressionEntryList) SetPrevious(v string) {
	o.Previous.Set(&v)
}
// SetPreviousNil sets the value for Previous to be an explicit nil
func (o *PaginatedSuppressionEntryList) SetPreviousNil() {
	o.Previous.Set(nil)
}

// UnsetPrevious ensures that no value is present for Previous, not even an explicit nil
func (o *PaginatedSuppressionEntryList) UnsetPrevious() {
	o.Previous.Unset()
}

// GetResults returns the Results field value
func (o *PaginatedSuppressionEntryList) GetResults() []SuppressionEntry {
	if o == nil {
		var ret []SuppressionEntry
		return ret
	}

	return o.Results
}

// GetResultsOk returns a tuple with the Results field value
// and a boolean to check if the value has been set.
func (o *PaginatedSuppressionEntryList) GetResultsOk() ([]SuppressionEntry, bool) {
	if o == nil {
		return nil, false
	}
	return o.Results, true
}

// SetResults sets field value
func (o *PaginatedSuppressionEntryList) SetResults(v []SuppressionEntry) {
	o.Results = v
}

func (o PaginatedSuppressionEntryList) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PaginatedSuppressionEntryList) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["count"] = o.Count
	if o.Next.IsSet() {
		toSerialize["next"] = o.Next.Get()
	}
	if o.Previous.IsSet() {
		toSerialize["previous"] = o.Previous.Get()
	}
	toSerialize["results"] = o.Results
	return toSerialize, nil
}

func (o *PaginatedSuppressionEntryList) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"count",
		"results",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varPaginatedSuppressionEntryList := _PaginatedSuppressionEntryList{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	err = decoder.Decode(&varPaginatedSuppressionEntryList)

	if err != nil {
		return err
	}

	*o = PaginatedSuppressionEntryList(varPaginatedSuppressionEntryList)

	return err
}

type NullablePaginatedSuppressionEntryList struct {
	value *PaginatedSuppressionEntryList
	isSet bool
}

func (v NullablePaginatedSuppressionEntryList) Get() *PaginatedSuppressionEntryList {
	return v.value
}

func (v *NullablePaginatedSuppressionEntryList) Set(val *PaginatedSuppressionEntryList) {
	v.value = val
	v.isSet = true
}

func (v NullablePaginatedSuppressionEntryList) IsSet() bool {
	return v.isSet
}

func (v *NullablePaginatedSuppressionEntryList) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePaginatedSuppressionEntryList(val *PaginatedSuppressionEntryList) *NullablePaginatedSuppressionEntryList {
	return &NullablePaginatedSuppressionEntryList{value: val, isSet: true}
}

func (v NullablePaginatedSuppressionEntryList) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePaginatedSuppressionEntryList) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


